...
Three tables. Nothing else earned its place inside the timebox.
...
The composite primary key on `document_shares` is doing real work too: it *is*
...
**What this is and is not.** This is *simulated identity*, not authentication.
...
character of the upload into a TipTap *text node*. There is no code path where
...
### D5 — No real-time collaboration
**Decision.** Documents are shared and independently editable; they are not
co-edited live.
...
can only ever attach the *caller's own* share row — joining on `document_id`
