...
const planningDoc: TipTapDoc = {
  ...
  content: [
    ...
    orderedList(
      item(bold("Import fidelity"), text(" — a document that arrives broken is never trusted again.")),
      item(bold("Sharing"), text(" — the moment a second person touches a document, it becomes a product.")),
      item(bold("Latency"), text(" — under 100ms to first keystroke, or it feels like a form.")),
    ),
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const seedDocuments = [
      ...
      { id: DOC_DESIGN_REVIEW, ownerId: GRACE, title: "Design Review Notes", content: designReviewDoc },
    ];
